...
export type InventoryRow = { part_id: string; quantity: number; min_quantity: number; location: string | null; part: { name: string; unit: string | null } | null };
...
export async function fetchInventory(): Promise<InventoryRow[]> {
  const { data, error } = await (supabase as any).from('inventory').select('part_id, quantity, min_quantity, location, part:part_id ( name, unit )').order('quantity', { ascending: true }).limit(200);
