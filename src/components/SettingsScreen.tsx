...
import {
  ...
  HardHat,
  PackageSearch,
  Wallet, Bell, Camera, ChevronLeft, ChevronRight, Clock3, Edit3, FileText, History, LockKeyhole, LogOut, Mail, MapPin, MessageSquare, Phone, RefreshCw, Settings, ShieldCheck, UserRound, Wifi } from 'lucide-react-native';
import { useRouter } from 'expo-router';
import { Image, Linking, Pressable, ScrollView, StyleSheet, Switch, View } from 'react-native';
...
import { fotoOuBoneco, removerFotoDePerfil, trocarFotoDePerfil, type OrigemDaFoto } from '@/services/perfil';
...
export function SettingsScreen({ roleLabel, subtitle }: SettingsScreenProps) {
  ...
  async function mudarFoto(origem: OrigemDaFoto) {
  ...
  function escolherOrigem() {
    Alert.alert('Foto de perfil', 'De onde quer tirar a foto?', [
      { text: 'Câmera', onPress: () => { void mudarFoto('camera'); } },
      { text: 'Galeria', onPress: () => { void mudarFoto('galeria'); } },
      ...(profile?.avatar_url
        ? [{
            text: 'Remover',
...
const styles = StyleSheet.create({
  fotoArea: { alignItems: 'center', gap: spacing.xs, paddingBottom: spacing.sm },
  foto: { width: 92, height: 92, borderRadius: 46, backgroundColor: colors.bgApp },
  fotoSelo: {
    position: 'absolute',
    top: 62,
    right: '50%',
