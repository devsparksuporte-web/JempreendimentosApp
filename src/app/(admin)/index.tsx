...
import { AlertCircle, BarChart3, CalendarClock, CheckCircle2, ChevronRight, CircleDollarSign, ClipboardList, FileCheck2, HardHat, LayoutGrid, MessageCircle, MonitorPlay, PackageCheck, PackageSearch, RefreshCw, Truck, UserRound } from 'lucide-react-native';
          ...
          {loading ? <LoadingState /> : error ? <ErrorState message={error} onRetry={load} /> : data ? (
            <>
              ...
              <View style={[styles.operationLinks, emGrade && styles.operationLinksEmGrade]}>
                ...
                <Pressable onPress={() => router.push('/(admin)/financeiro' as never)} style={({ pressed }) => [styles.operationLink, emGrade && styles.operationLinkEmGrade, pressed && styles.pressed]}><CircleDollarSign size={20} color={colors.brandStrong} /><View style={styles.flex}><Text variant="bodyStrong">Financeiro</Text><Text variant="meta" color={colors.textSecondary}>Contratos, faturamento e contas a receber</Text></View><ChevronRight size={18} color={colors.slate300} /></Pressable><Pressable onPress={() => router.push('/(admin)/relatorios' as never)} style={({ pressed }) => [styles.operationLink, emGrade && styles.operationLinkEmGrade, pressed && styles.pressed]}><BarChart3 size={20} color={colors.brandStrong} /><View style={styles.flex}><Text variant="bodyStrong">Relatórios</Text><Text variant="meta" color={colors.textSecondary}>Performance e analytics</Text></View><ChevronRight size={18} color={colors.slate300} /></Pressable>
