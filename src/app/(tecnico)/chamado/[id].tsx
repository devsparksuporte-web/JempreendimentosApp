...
import { captureAndUploadPhoto, fetchChecklist, fetchChecklistResults, fetchServicePhotos, fetchTechnicianCall, saveChecklistResult, updateTechnicianStatus, type ChecklistItem, type TechnicianCall, type TechnicianPhoto } from '@/services/technician';
...
export default function TechnicianCallScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setCall(nextCall); setItems(checklist); setPhotos(nextPhotos); setResults(Object.fromEntries(saved.map((item) => [item.checklist_item_id, item.checked])));
  ...
  return <View style={styles.root}>
    ...
    <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
      <View style={styles.container}>
        ...
        <View style={styles.actions}>{call.status === 'tecnico_atribuido' || call.status === 'aguardando_tecnico' ? <Button label="Iniciar deslocamento" icon={Play} onPress={() => changeStatus('a_caminho')} loading={busy} /> : null}{call.status === 'a_caminho' ? <Button label="Iniciar atendimento" icon={Play} onPress={() => changeStatus('em_atendimento')} loading={busy} /> : null}{call.status === 'em_atendimento' ? <Button label={checklistComplete && beforeCount > 0 && afterCount > 0 ? 'Finalizar atendimento' : 'Complete checklist e fotos'} icon={Square} onPress={() => changeStatus('finalizado')} loading={busy} disabled={!checklistComplete || beforeCount === 0 || afterCount === 0} /> : null}</View>
      </View>
    </ScrollView>
  </View>;
}
